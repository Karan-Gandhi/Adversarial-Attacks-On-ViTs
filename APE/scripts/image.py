...
# from eval.models.image.cct import ourCCT
...
from ape_local.nn.schedule import make_schedule
...
def run(
        model: Literal['Unitary', 'Sinusoidal', 'Absolute', 'UnitarySeq',"Ours"],
        num_epochs: int,
        num_layers: int,
        num_heads: int,
        dim: int,
    ...
    for epoch in range(num_epochs):
        model.train()
        print(f'{epoch}')
        epoch_loss, batch_correct, total_correct = 0, 0, 0
        ...
        print(f'Accuracy (m) {total_correct / len(train_set):.4f}')

        model.eval()
        val_loss, total_correct = 0, 0
        with torch.no_grad():
        ...
        dev_acc = total_correct / len(test_set)
        print(f'Test loss {val_loss:.4f}')
        print(f'Dev acc (token) {dev_acc:.4f}')
...
def parse_args():
    ...
    parser.add_argument('--model', type=str, required=True, choices=['Algebraic', 'Sinusoidal', 'Absolute','Ours', 'AlgebraicSeq'], help='Type of model to use')
    parser.add_argument('--data_dir', type=str, required=True, help='Where is the data located')
    parser.add_argument('--num_epochs', type=int, default=54, help='Number of training epochs')

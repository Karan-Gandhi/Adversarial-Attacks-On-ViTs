...
class LearnableSinCos2DPositionalEncoding(nn.Module):
    """2D Sinusoidal Positional Encoding for Vision Transformer (Add mode)"""
    
    def __init__(self, dim, h=8, w=8):
        super(LearnableSinCos2DPositionalEncoding, self).__init__()
        ...
        div_term_h_params = nn.Parameter(torch.zeros(dim_h // 2))
        div_term_w_params = nn.Parameter(torch.zeros(dim_w // 2))
        # Create division term for computing positional encoding values
        div_term_h = torch.exp(torch.arange(0, dim_h, 2).float() * -(math.log(10000.0) / dim_h)) + div_term_h_params
        div_term_w = torch.exp(torch.arange(0, dim_w, 2).float() * -(math.log(10000.0) / dim_w)) + div_term_w_params
